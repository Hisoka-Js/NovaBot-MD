let handler = async (m, { conn, isOwner, text, isAdmin }) => {
  ...
  try {
    ...
    m.reply(`chat baneado! ${await conn.user.name} no estaba activo el este chat ${await conn.getName(who) == undefined ? 'ini' : await conn.getName(who)}.`)
  } catch (e) {
    throw `numero no existe el base del datos!`
  }
}
handler.help = ['ban']
